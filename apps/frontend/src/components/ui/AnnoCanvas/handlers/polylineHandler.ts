...
/**
 * Polyline 어노테이션 핸들러 클래스
 */
export class PolylineHandler implements AnnotationHandler<PolylineAnnotation> {
  private callbacks: HandlerCallbacks<PolylineAnnotation> | null = null;

  setCallbacks(callbacks: HandlerCallbacks<PolylineAnnotation>): void {
  ...
  handleFinish(data: PointBasedFinishData): boolean {
    ...
    this.callbacks.onFinishDrawing(polyline);
    return true;
  }
  ...
  handleMove(
    ...
    stageSize: { width: number; height: number },
    pointerPos: { x: number; y: number } | null,
    _state: PolylineMoveState // eslint-disable-line @typescript-eslint/no-unused-vars
  ): boolean {
    if (!pointerPos || !this.callbacks) return false;
